...
image = cv2.imread('lectures/src/arrow.png')
...
for p in approx:
    cv2.circle(image, tuple(*p), 6, (0, 255, 255), 2)

# построить гробик - замнутая фигура с мин количеством точек
hull = cv2.convexHull(arrow)
for i in range(1, len(hull)):
    cv2.line(image, tuple(*hull[i-1]), tuple(*hull[i]), (255, 255, 0), 2)
cv2.line(image, tuple(*hull[-1]), tuple(*hull[0]), (255, 255, 0), 2)

# построить прямоугольник по точкам
...
approx = cv2.approxPolyDP(arrow, eps, True)
for p in approx:
    cv2.circle(image, tuple(*p), 6, (0, 255, 255), 7)
